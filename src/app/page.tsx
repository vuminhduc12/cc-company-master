  ...
  return (
    <div className="space-y-5 sm:space-y-7">
      ...
      <section className="overflow-hidden rounded-2xl border border-white/10 bg-[linear-gradient(135deg,rgba(23,23,23,0.96),rgba(5,5,5,0.98)_55%,rgba(19,78,74,0.60))] text-slate-50 shadow-2xl shadow-black/35 ring-1 ring-white/5">
        ...
        <div className="grid gap-4 p-3 sm:p-4 lg:grid-cols-[minmax(0,1fr)_360px] lg:p-6 xl:grid-cols-[minmax(0,1fr)_420px]">
          <div className="min-w-0">
            <div className="border-b border-white/10 pb-5">
              <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-4xl font-black tracking-tight text-white sm:text-7xl">{selectedItem.stock.ticker}</h2>
                  ...
                  <div className="mt-4 flex flex-wrap items-end gap-2 sm:gap-3">
                    <p className="text-3xl font-black tracking-tight text-white tabular-nums sm:text-6xl">
                ...
                <div className="flex w-full flex-col gap-3 md:w-auto md:shrink-0 md:items-end">
                  {/* ⑦ 円形スコアゲージ (⑨ カウントアップ適用) */}
                  <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border border-teal-300/20 bg-teal-300/[0.08] px-4 py-3 md:hidden">
                    <div>
                      <p className="text-[11px] font-black uppercase tracking-[0.16em] text-teal-200">AI Score</p>
                      <p className="mt-1 text-sm font-semibold text-slate-400">現在の優先監視度</p>
                    </div>
                    <p className="text-3xl font-black text-white tabular-nums">{animScore}</p>
                  </div>
                  <div className="hidden w-full items-center justify-center rounded-2xl border border-teal-300/20 bg-teal-300/[0.06] py-3 shadow-2xl shadow-teal-950/20 md:flex md:w-[160px]">
                    <ScoreGauge score={animScore} maxScore={100} size={120} />
                  </div>
                  <div className="grid grid-cols-2 gap-2 md:flex">
                    <Link className="rounded-lg border border-white/10 bg-white/[0.08] px-4 py-2 text-center text-sm font-bold text-slate-100 shadow-sm hover:bg-white/[0.12]" href={`/stocks/${selectedItem.stock.ticker}`}>
                      詳細
                    </Link>
                    <a className="rounded-lg border border-teal-300/25 bg-teal-300/10 px-4 py-2 text-center text-sm font-bold text-teal-100 shadow-sm hover:bg-teal-300/[0.15]" href={selectedFinanceUrl} target="_blank" rel="noreferrer">
                      Google Finance
                    </a>
                  </div>
                </div>
          ...
          <aside className="min-w-0 space-y-4">
            ...
            <div className="hidden rounded-2xl border border-white/10 bg-black/28 p-5 shadow-xl shadow-black/20 ring-1 ring-white/5 lg:block">
      ...
      <section className="grid min-w-0 gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        <div className="min-w-0 space-y-4">
          ...
          <div className="max-h-[360px] space-y-2 overflow-y-auto pr-1 [-webkit-overflow-scrolling:touch] xl:max-h-none xl:overflow-visible xl:pr-0">
            {marketFilteredWatchlist.map((item) => {
              const quote = quoteForWatchItem(item, { selectedTicker, realtimeQuote });
