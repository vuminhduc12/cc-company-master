...
export function AiEmployeeCard({ task }: { task: AiTask }) {
  return (
    <article className="rounded-2xl border border-white/10 bg-slate-900/75 p-4 shadow-lg shadow-black/20">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="font-semibold text-slate-50">{task.name}</h3>
          <p className="mt-1 text-xs text-sky-200">{task.role}</p>
        </div>
        <StatusBadge value={task.status} />
      </div>
      <p className="mt-3 text-sm text-slate-300">{task.task}</p>
      <div className="mt-4 rounded-xl bg-slate-950/50 p-3">
        <p className="text-xs text-slate-500">Result</p>
        <p className="mt-1 text-sm text-slate-100">{task.result}</p>
