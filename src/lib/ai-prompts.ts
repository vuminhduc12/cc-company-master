export const aiPromptVersions = {
  spotSimulatorNormal: "spot-simulator-normal-v2.2",
  spotSimulatorDetailed: "spot-simulator-detailed-v3.1",
...
function buildDetailedSystemPrompt() {
  return [
    ...
    "Analyze macro impact after news impact and before scenario forecast. Explain whether the macro regime is supportive, neutral, or a headwind for this specific stock, with special attention to growth/high-valuation/small-cap/cyclical sensitivity.",
    ...
    "Return strict JSON in Japanese with keys: summary, dataFreshness, riskLevel, confidence, marketView, historicalPatternView, newsImpactView, macroImpactView, largeMoneyFlowView, scenarioForecast, riskFilter, analystView, scenarioPrediction, userQuestionAnswer, entryPriceComment, positionSizeComment, exitPlanComment, taxComment, fxComment, technicalComment, newsComment, stressTest, blindSpots, checklist.",
    ...
    "historicalPatternView, newsImpactView, macroImpactView, largeMoneyFlowView, and riskFilter must not repeat the same wording as analystView or scenarioPrediction.",
    "summary must be concise but include the dominant risk and top risk drivers, not a restatement of numbers.",
    "analystView must be 6 to 9 short Japanese sentences and must explicitly mention: priority scenario, short-term range view, medium-term condition, upside condition, danger level, large-money-flow interpretation, macro interpretation, news interpretation, past similar-pattern evidence, and a short Buffett-style risk filter.",
