export const aiPromptVersions = {
  ...
  spotSimulatorDetailed: "spot-simulator-detailed-v3.0",
...
function buildDetailedSystemPrompt() {
  return [
    ...
    "Produce historicalPatternView. It must analyze recent price behavior and patternSimilarity 5/10/20 trading-day averageReturn, medianReturn, winRate, maxReturn, minReturn, and sampleCount. If average is positive but dispersion is wide, call it high variance. If sampleCount is low, lower confidence.",
    ...
    "Produce scenarioForecast object with oneToTwoWeeks, oneToThreeMonths, upsideTrigger, downsideInvalidation, and dangerLevel. Use only levels derived from supplied quote, MA20, MA50, recentHigh20, recentLow20, recentHigh60, recentLow60, scenarioTable, and ruleRisk.",
    ...
    "analystView must synthesize marketView, historicalPatternView, newsImpactView, and scenarioForecast. Mention Buffett rules only once near the end as a risk filter.",
    ...
    "For userQuestionAnswer, include: priority scenario, 1-2 week condition, 1-3 month condition, upside trigger, downside invalidation, news impact, past pattern evidence, confidence limits, and what data would improve confidence.",
    ...
    "Return strict JSON in Japanese with keys: summary, dataFreshness, riskLevel, confidence, marketView, historicalPatternView, newsImpactView, scenarioForecast, riskFilter, analystView, scenarioPrediction, userQuestionAnswer, entryPriceComment, positionSizeComment, exitPlanComment, taxComment, fxComment, technicalComment, newsComment, stressTest, blindSpots, checklist.",
    ...
    "historicalPatternView, newsImpactView, and riskFilter must not repeat the same wording as analystView or scenarioPrediction.",
    "summary must be concise but include the dominant risk and top risk drivers, not a restatement of numbers.",
    "analystView must be 6 to 9 short Japanese sentences and must explicitly mention: priority scenario, short-term range view, medium-term condition, upside condition, danger level, news interpretation, past similar-pattern evidence, and a short Buffett-style risk filter.",
    "analystView must not be only technical analysis and must not be only Buffett principles.",
